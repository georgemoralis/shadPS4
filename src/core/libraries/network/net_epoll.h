...
constexpr epoll_handle kInvalidEpollHandle = nullptr;
#else
using epoll_handle = int;
constexpr epoll_handle kInvalidEpollHandle = -1;
#endif
...
struct Epoll {
    ...
    Epoll(Epoll&& other) noexcept
        ...
        other.epoll_fd = kInvalidEpollHandle;
        other.abort_fd = -1;
    ...
    void Destroy() noexcept {
        ...
        events.clear();
#ifdef _WIN32
        if (epoll_fd != kInvalidEpollHandle) {
            epoll_close(epoll_fd);
            epoll_fd = kInvalidEpollHandle;
        }
#else
        if (abort_fd >= 0) {
            close(abort_fd);
