...
int PS4_SYSV_ABI sceNetEpollAbort(OrbisNetId epollid, int flags) {
    LOG_DEBUG(Lib_Net, "called, epollid = {}, flags = {}", epollid, flags);
...
int PS4_SYSV_ABI sceNetEpollWait(OrbisNetId epollid, OrbisNetEpollEvent* events, int maxevents,
    ...
    if (is_p2p_epoll) {
        ...
        g_p2p_epoll_wait_count.fetch_add(1, std::memory_order_relaxed);
    }
    ...
    if (is_p2p_epoll && result >= 0) {
        ...
        for (const auto& [orbis_fd, ep_event] : epoll->events) {
            ...
            if (file->socket && file->socket->HasQueuedData()) {
                const auto* p2p_socket =
                    dynamic_cast<Libraries::Net::P2PSocket*>(file->socket.get());
                if (p2p_socket) {
                    LOG_ERROR(Lib_Net,
                              "P2P epoll synth: orbis_fd={} guest_name={} bound_vport={} "
                              "queued=true data_u64={:#x}",
                              orbis_fd, file->m_guest_name, ntohs(p2p_socket->bound_vport_),
                              ep_event.data.data_u64);
                } else {
                    LOG_ERROR(Lib_Net,
                              "P2P epoll synth: orbis_fd={} guest_name={} queued=true "
                              "data_u64={:#x}",
                              orbis_fd, file->m_guest_name, ep_event.data.data_u64);
                }
                events[p2p_count] = {
