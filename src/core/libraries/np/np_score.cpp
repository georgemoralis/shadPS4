...
int PS4_SYSV_ABI sceNpScoreCreateNpTitleCtx(OrbisNpServiceLabel serviceLabel, OrbisNpId* npId) {
    LOG_INFO(Lib_NpScore, "(STUBBED) serviceLabel = {}, npId->data = {}", serviceLabel,
             npId->handle.data);
    return ORBIS_OK;
}

int PS4_SYSV_ABI sceNpScoreCreateRequest(s32 titleCtxId) {
    LOG_INFO(Lib_NpScore, "(STUBBED) libCtxId = {}", titleCtxId);
    return ORBIS_OK;
}

int PS4_SYSV_ABI sceNpScoreDeleteRequest(s32 reqId) {
    LOG_INFO(Lib_NpScore, "(STUBBED) requestId = {:#x}", reqId);
    return ORBIS_OK;
}
...
int PS4_SYSV_ABI sceNpScoreGetRankingByRange(
    ...
    LOG_INFO(Lib_NpScore, "(STUBBED) called reqId={}, boardId={}, startSerialRank={}, arrayNum={}",
