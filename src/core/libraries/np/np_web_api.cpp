...
s32 PS4_SYSV_ABI sceNpWebApiCreateRequest(s32 title_user_ctx_id, const char* p_api_group,
    ...
    LOG_ERROR(Lib_NpWebApi,
              "CreateRequest: ctxId={} group='{}' path='{}' method={} contentParam={:p}",
              title_user_ctx_id, p_api_group ? p_api_group : "(null)", p_path ? p_path : "(null)",
              method, static_cast<void*>(p_content_parameter));

    if (p_content_parameter) {
        LOG_ERROR(Lib_NpWebApi, "CreateRequest: contentType='{}' contentLength={}",
                  p_content_parameter->p_content_type ? p_content_parameter->p_content_type
                                                      : "(null)",
                  p_content_parameter->content_length);
    }
...
s32 PS4_SYSV_ABI sceNpWebApiGetHttpStatusCode(s64 request_id, s32* out_status_code) {
    ...
    if (it != g_requests.end()) {
        *out_status_code = 200;
        ...
        // Unmocked requests (sessions, presence, etc.) — return 200 so the
        // game's HTTP polling loop completes instead of spinning forever.
        *out_status_code = 200;
    }
...
s32 PS4_SYSV_ABI sceNpWebApiSendRequest(s64 request_id, const void* body, u64 body_size) {
    LOG_ERROR(Lib_NpWebApi, "SendRequest: request_id={} body={:p} body_size={}", request_id, body,
              body_size);
    if (body && body_size > 0 && body_size < 4096) {
        // Log first 256 bytes of body as hex for debugging
        auto* p = static_cast<const u8*>(body);
        u64 log_len = std::min(body_size, static_cast<u64>(256));
        std::string hex;
        for (u64 i = 0; i < log_len; ++i) {
            char buf[4];
            snprintf(buf, sizeof(buf), "%02x ", p[i]);
            hex += buf;
        }
        LOG_ERROR(Lib_NpWebApi, "SendRequest body[0..{}]: {}", log_len, hex);
    }
    return ORBIS_OK;
}
